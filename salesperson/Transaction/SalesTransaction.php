...
<script>
    ...
    function addToCart(ProductId, ProductName, RetailPrice, Images) {
        ...
        // Sửa lại tên trường là ProductPrice
            ...
            <div class="customer">
                <div class="info">
                    ...
                    <div class="operation_actived">
                        ...
                        <h5>${RetailPrice}$</h5>
                        <span><button onclick="deleteProduct(${ProductId})">Delete</button></span>
                    </div>
                </div>
            </div>
        `;
    ...
    function deleteProduct(ProductId) {
        var productContainer = document.querySelector(`.card-body[data-product-id="${ProductId}"]`);
        var productItem = document.getElementById('checkoutForm');

        console.log(productContainer);

        if (productContainer) {
            var productIdInput = productItem.querySelector(`input[name="productId[]"][value="${ProductId}"]`);
            var productNameInput = productItem.querySelector(`input[name="productName[]"][value="${productContainer.querySelector('.operation_actived h4').textContent}"]`);

            var retailPriceStr = productContainer.querySelector('.operation_actived h5:nth-child(3)').textContent; 
            var retailPrice = parseFloat(retailPriceStr.replace('$', ''));
            var productPriceInput = productItem.querySelector(`input[name="productPrice[]"][value="${retailPrice}"]`);
            
            var productImageInput = productItem.querySelector(`input[name="productImage[]"][value="${productContainer.querySelector('img').getAttribute('src')}"]`);
            ...
            if (productNameInput) productNameInput.remove();
            if (productPriceInput) productPriceInput.remove();
            if (productImageInput) productImageInput.remove();
...
<body>
    <form id="checkoutForm" action="SalesCart.php?username=<?php echo urlencode($_GET['username']); ?>" method="post">
                ...
                <div class="cards">
                    ...
                    <div class="card-single">
                        ...
                        <div class="product-cost card-header">
                            <?= $row['RetailPrice'] ?>
                            <a class ="sidebar-link" href="SalesProductDetails.php?ProductID=<?= $row['ProductID'] ?>"><button> More <label class="las la-arrow-right"></label></button></a>
                        </div>
                     </div>   
                    <?php 
                            }
